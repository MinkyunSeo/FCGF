...
class ResUNet2(ME.MinkowskiNetwork):
  ...
  def forward(self, x):
    ...
    out_s2_tr = MEF.relu(out) #coarse

    out = ME.cat(out_s2_tr, out_s2)
    out = self.conv2_tr(out)
    out = self.norm2_tr(out)
    out = self.block2_tr(out)
    out_s1_tr = MEF.relu(out) #mid
    ...
    if self.resolution == 'fine':
      ...
      # For 16 dim
      # avg_feat = out_s1_tr.F.view(-1, 4, 16).mean(dim=1).view(-1, 16)
      # out = ME.SparseTensor(features=avg_feat, coordinate_map_key=out_s1_tr.coordinate_map_key, coordinate_manager=out_s1_tr.coordinate_manager)
